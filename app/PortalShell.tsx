...
export function AdminPortal() {
  ...
  const [removingCustomer, setRemovingCustomer] = useState<AdminCustomerRow | null>(null);
  ...
  return <PortalFrame role="admin" active={active} setActive={setActive}>
    ...
    {removingCustomer && <RemoveCustomerDialog customer={removingCustomer} onClose={() => setRemovingCustomer(null)} onRemoved={() => { setRemovingCustomer(null); setLoading(true); loadAdminData(); }} />}
  </PortalFrame>;
}
...
function AdminOverview({ setActive, onAdd, customers, managedAccounts, pendingRequests, renewalsDue, activeValue, loading }: { setActive: (id: string) => void; onAdd: () => void; customers: AdminCustomerRow[]; managedAccounts: number; pendingRequests: number; renewalsDue: AdminCustomerRow[]; activeValue: number; loading: boolean }) {
  ...
  const subscribedCount = customers.filter((row) => row.recordKind === "internal" || Boolean(row.subscriptionId)).length;
...
function CustomerTable({ rows, loading = false, onManagePlan, onManageAccounts, onManagePrefix, onRemove }: { rows: AdminCustomerRow[]; loading?: boolean; onManagePlan?: (customer: AdminCustomerRow) => void; onManageAccounts?: (customer: AdminCustomerRow) => void; onManagePrefix?: (customer: AdminCustomerRow) => void; onRemove?: (customer: AdminCustomerRow) => void }) {
...
function RemoveCustomerDialog({ customer, onClose, onRemoved }: { customer: AdminCustomerRow; onClose: () => void; onRemoved: () => void }) {
  const [confirmation, setConfirmation] = useState("");
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");

  async function remove() {
    if (confirmation.trim() !== customer.name) { setMessage(`Type ${customer.name} exactly to confirm.`); return; }
    const supabase = getSupabaseBrowserClient();
    if (!supabase) { setMessage("The secure database connection is not available."); return; }
    const { data } = await supabase.auth.getSession();
    setBusy(true); setMessage("");
    const response = await fetch("/api/admin/customers/remove", { method: "DELETE", headers: { "content-type": "application/json", authorization: `Bearer ${data.session?.access_token ?? ""}` }, body: JSON.stringify({ recordKind: customer.recordKind, customerId: customer.recordKind === "internal" ? customer.internalRecordId : customer.userId }) });
    const result = await response.json().catch(() => ({ error: "The server returned an unreadable response." })) as { error?: string };
    if (!response.ok) { setBusy(false); setMessage(typeof result.error === "string" ? result.error : "Could not remove this customer."); return; }
    setBusy(false); onRemoved();
  }

  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="remove-customer-title" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><p className="eyebrow">Permanent removal</p><h2 id="remove-customer-title">Remove {customer.name}?</h2></div><button className="icon-btn" onClick={onClose} aria-label="Close removal dialog"><X /></button></div><p className="muted">{customer.recordKind === "internal" ? "This permanently removes the private subscription record and its renewal history." : "This permanently removes the customer login, subscription, managed accounts and settings requests."}</p><div className="removal-warning"><Trash2 /><span><strong>This cannot be undone</strong><small>Type the customer name exactly before removing the record.</small></span></div><div className="support-form"><label>Type “{customer.name}” to confirm<input value={confirmation} onChange={(event) => setConfirmation(event.currentTarget.value)} autoComplete="off" /></label><div className="plan-dialog-actions"><button className="secondary-button" onClick={onClose} disabled={busy}>Cancel</button><button className="secondary-button danger" onClick={remove} disabled={busy || confirmation.trim() !== customer.name}>{busy ? "Removing…" : "Remove customer permanently"}</button></div></div>{message && <p className="form-message" role="status">{message}</p>}</section></div>;
}
