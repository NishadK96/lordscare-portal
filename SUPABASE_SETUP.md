...
4. Copy `.env.example` to `.env.local` and add the project URL, anon key and service-role key from Supabase Project Settings → API. The service-role key is used only by the protected server endpoint that invites customers.
